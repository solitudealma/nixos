...
function show_status() {
	...
	sed -i "/^statusbar_visibility/ s/\".*/\"${status:-no}\"/" ~/.config/ncmpcpp/config{,_cover_art}
}
...
function draw_cover_art() {
	cover=$(~/.config/ncmpcpp/scripts/get_cover_art.sh)
	[[ -f "$cover" ]] && args="draw $x $y $cover_width $cover_height $cover"
	command="~/.config/ncmpcpp/scripts/ueberzug_wrapper.sh $args"
	echo "~/.config/ncmpcpp/scripts/ueberzug_wrapper.sh $args" > ~/ub.log

	tmux_name="ncmpcpp_with_cover_art"
	tmux_base="tmux -S /tmp/tmux_hidden"

	if ((pane_count == 1)); then
		initialize='export blank=true && source ~/.bashrc && ~/.config/ncmpcpp/scripts/ueberzug_parser.sh'
...
while getopts :pvscdaRVCP:S:L:D:r:w:h:i flag; do
	case $flag in
		p)
			...
			[[ ! $pre ]] && pre="~/.config/ncmpcpp/scripts/windowctl.sh "
		...
		c)
			...
			command="new -s $title ~/.config/ncmpcpp/scripts/ueberzug_parser.sh \; "
			command+="splitw -${orientation}${back}l 80% ncmpcpp -c ~/.config/ncmpcpp/config_cover_art";;
		d)
			~/.config/ncmpcpp/scripts/ncmpcpp.sh $display $V -v -i
			until [[ $(wmctrl -l | awk '$NF ~ "visualizer"') ]]; do continue; done
			~/.config/ncmpcpp/scripts/ncmpcpp.sh $display $V -p -i
			exit;;
		C)
			arg=${!OPTIND}
		...
		[PS])
			...
			(($# + 1 == OPTIND)) && exit 0;;
		L) pre="~/.config/ncmpcpp/scripts/windowctl.sh $OPTARG";;
		D)
			display="-D $OPTARG"
			pre="~/.config/ncmpcpp/scripts/windowctl.sh -d $OPTARG move";;
		V) V=-V;;
		R)
			ratio=$(sed -n 's/^execute.*[^0-9]\([0-9]\+\).*/\1/p' ~/.config/ncmpcpp/config_cover_art)
		...
		i)
			if ! wmctrl -a "${title-ncmpcpp}"; then
				...
				if [[ $mode == tiling ]]; then
					if grep "^tiling.*\b$workspace\b" ~/.config/ncmpcpp/scripts/spy_windows.sh &> /dev/null; then
						[[ $title == visualizer ]] &&
